...
pack_mcmod = mcmod_info.each_with_object({}) do |mcmod, result|
  ...
  packname = if packname == 'wiccanArts'
    'core'
  else
    packname.gsub('wiccanArts|', '')
  end
...
# for it
Dir.glob File.expand_path("../build/libs/wiccanArts-*.jar", File.dirname(__FILE__)) do |f|
