...
public class BlockChalkGlyph extends BlockMod {

	public static final PropertyEnum<BlockChalkGlyph.EnumAttachPosition> NORTH = PropertyEnum.create("north", BlockChalkGlyph.EnumAttachPosition.class);
	public static final PropertyEnum<BlockChalkGlyph.EnumAttachPosition> EAST = PropertyEnum.create("east", BlockChalkGlyph.EnumAttachPosition.class);
	public static final PropertyEnum<BlockChalkGlyph.EnumAttachPosition> SOUTH = PropertyEnum.create("south", BlockChalkGlyph.EnumAttachPosition.class);
	public static final PropertyEnum<BlockChalkGlyph.EnumAttachPosition> WEST = PropertyEnum.create("west", BlockChalkGlyph.EnumAttachPosition.class);
	...
	public BlockChalkGlyph() {
		super(LibBlockName.CHALK_GLYPH, Material.CIRCUITS);
		this.setDefaultState(this.blockState.getBaseState().withProperty(NORTH, BlockChalkGlyph.EnumAttachPosition.NONE).withProperty(EAST, BlockChalkGlyph.EnumAttachPosition.NONE).withProperty(SOUTH, BlockChalkGlyph.EnumAttachPosition.NONE).withProperty(WEST, BlockChalkGlyph.EnumAttachPosition.NONE));
		setSound(SoundType.CLOTH);
	}

	private static boolean canConnectTo(IBlockState blockState) {
		final Block block = blockState.getBlock();
		return block == ModBlocks.chalk_glyph;
	}
	...
	private static int getAABBIndex(IBlockState state) {
		int i = 0;
		final boolean flag = state.getValue(NORTH) != BlockChalkGlyph.EnumAttachPosition.NONE;
		final boolean flag1 = state.getValue(EAST) != BlockChalkGlyph.EnumAttachPosition.NONE;
		final boolean flag2 = state.getValue(SOUTH) != BlockChalkGlyph.EnumAttachPosition.NONE;
		final boolean flag3 = state.getValue(WEST) != BlockChalkGlyph.EnumAttachPosition.NONE;
	...
	private BlockChalkGlyph.EnumAttachPosition getAttachPosition(IBlockAccess worldIn, BlockPos pos, EnumFacing direction) {
			...
			if (!iblockstate1.isNormalCube()) {
				...
				if (flag && canConnectUpwardsTo(worldIn, blockpos.up())) {
					if (iblockstate.isBlockNormalCube()) {
						return BlockChalkGlyph.EnumAttachPosition.UP;
					}

					return BlockChalkGlyph.EnumAttachPosition.SIDE;
				}
			}

			return BlockChalkGlyph.EnumAttachPosition.NONE;
		} else {
			return BlockChalkGlyph.EnumAttachPosition.SIDE;
		}
	}
