...
public class ItemSalt extends ItemMod {
	...
	public EnumActionResult onItemUse(Block block, ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (playerIn.canPlayerEdit(blockpos, facing, stack) && block.canPlaceBlockAt(worldIn, pos) && ModBlocks.SALT_BARRIER.canPlaceBlockAt(worldIn, blockpos)) {
			stack.getItem();
